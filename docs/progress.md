...
- P1-03 moved to Review and completed: map checkpoints are now compact, configuration-driven circular nodes with responsive percentage positions, keyboard-accessible names, and hover/focus tooltips. The current lesson remains the sole in-map card CTA; mobile retains only its clear primary CTA. Visual QA passed at 1680×900, 768×1024, and 390×844. Verification: `npm run check` passed (13 test files/46 tests, lint, typecheck, production build), `npm run test:e2e` passed 2 Chromium flows, and `git diff --check` passed.
- P1-03 resumed: replace large in-map lesson CTA cards with responsive checkpoint nodes. Keep one compact current-lesson CTA, preserve keyboard labels, and drive node positions from a shared checkpoint configuration.
- P1-03 moved to Review and completed: viewport sizing now belongs to the dashboard container rather than the hero. At desktop/tablet, the map flexes into the height remaining after the summary-card row; tablet also reserves the fixed bottom navigation height. Visual QA confirms the full dashboard fits at 1680×900 and 768×1024. Verification: `npm run check` passed (13 test files/46 tests, lint, typecheck, production build), `npm run test:e2e` passed 2 Chromium flows, and `git diff --check` passed.
- P1-03 resumed: revise viewport sizing to fit the whole dashboard — map and summary cards — rather than allowing the hero alone to consume the viewport.
- P1-03 moved to Review and completed: tablet (≥768px) hero height now uses `calc(100dvh - 7rem)` to account for the mobile/tablet header and page padding; desktop retains its `calc(100dvh - 3rem)` rule. Visual QA passed at 1024×768 and 768×1024 with all map CTAs visible. Verification: `npm run check` passed (13 test files/45 tests, lint, typecheck, production build), `npm run test:e2e` passed 2 Chromium flows, and `git diff --check` passed.
- P1-03 resumed: extend viewport-height map behavior to tablet, accounting for the tablet header and page padding while retaining the desktop height rule.
- P1-03 moved to Review and completed: desktop `LearningMapStage` now uses `calc(100dvh - 3rem)` for its hero height, with `object-cover` preserving image proportions. At 1680×900, visual QA confirmed the hero fits the viewport with every CTA visible. Verification: `npm run check` passed (13 test files/45 tests, lint, typecheck, production build), `npm run test:e2e` passed 2 Chromium flows, and `git diff --check` passed.
- P1-03 resumed: desktop hero height will follow the viewport while mobile and tablet preserve the map's natural aspect ratio.
- P1-03 moved to Review and completed: desktop AppShell now hides its mobile-only header and removes the desktop content-width cap, so the hero starts directly beneath the viewport edge and fills the entire area after the 230px sidebar. Verification: `npm run check` passed (13 test files/45 tests, lint, typecheck, production build), `npm run test:e2e` passed 2 Chromium flows, `git diff --check` passed, and visual QA at 1680px showed no top or right blank space.
- P1-03 resumed: removing desktop-only blank space above and beside the dashboard hero. Cause: the mobile header remained rendered at desktop widths and the content area retained a `max-w-6xl` cap after the sidebar was introduced.
- P1-03 moved to Review: updated the desktop dashboard against Figma node `7:275` with Nunito-led dashboard typography, a 230px navigation rail, polished mission/progress/companion cards, and stronger desktop/mobile CTAs. The existing responsive map hero and routes remain unchanged. Visual QA passed at desktop and 390px mobile widths; mobile companion artwork now flows below its copy to prevent text overlap.
- P1-03 completed: batch verification passed in `apps/web` — `npm run check` (lint, typecheck, 13 test files/44 tests, production build), `npm run test:e2e` (2 Chromium flows), and `git diff --check`.
- P1-03 resumed: aligning the existing student dashboard with the supplied Figma desktop design. Scope is typography, dashboard cards, shell navigation, and CTA polish only; preserve the responsive map hero and existing learning routes.

- P1-03 resumed: replacing the map image's baked-in lesson controls with responsive, keyboard-accessible CTA overlays that match the approved responsive mockup. Assumption: this is a corrective extension of the existing UI-foundation task because it directly fixes its desktop map surface.
- P1-03 moved to Review: generated a text-free 1672×941 game-map hero at `apps/web/public/dashboard/map-stage-v2.png`; added visible responsive map CTAs, progress card, learning stages, and a disabled locked stage in `LearningMapStage`. The new component test passed RED→GREEN. Verification: `npm run check` passed (13 test files/42 tests, lint, typecheck, production build); `npm run test:e2e` passed 2 Chromium flows; a Desktop Chrome screenshot was inspected at 1280px wide.
- P1-03 completed: next ready work remains blocked behind approved pilot content and live Supabase/RLS verification; the map UI is ready for content wiring once those gates are resolved.
- P1-03 reopened during visual QA: the responsive map no longer needs horizontal panning, so its obsolete mobile drag instruction is being removed.
- P1-03 final review: independent review found and the implementation fixed two important issues—offscreen mobile CTA and 14px student-facing map labels. Mobile visual QA at 375px confirms the primary CTA is fully visible without panning and obsolete drag guidance is removed. Verification rerun: `npm run check` passed (13 test files/43 tests, lint, typecheck, production build); `npm run test:e2e` passed 2 Chromium flows; `git diff --check` passed.
- P1-03 completed: native map CTAs now follow the approved mock's game-map interaction language on desktop and preserve one immediately visible primary CTA on mobile.
